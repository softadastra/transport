...
Sync operations are wrapped into transport messages.

The sync module does not know whether messages are sent through TCP, HTTP, WebSocket, LAN, or P2P.

### Protocol-light

The transport layer only understands message type, sender, recipient, correlation id, and payload bytes.

Payload meaning belongs to higher layers.
...
```text id="transport-frame-format"
uint32 payload_size
bytes  payload
...
TransportConfig controls bind address, timeouts, frame limits, and keepalive behavior.
...
TransportMessage is the logical message exchanged between peers.
...
auto ack =
    transport::core::TransportMessage::ack(
        "node-a",
        "sync-123");
...
TransportEnvelope wraps a TransportMessage with delivery metadata.
...
Frame represents a length-prefixed payload.
...
TransportAck is the payload used to acknowledge sync operations.
...
Encode and decode ack payloads through the dispatcher helpers:
...
PeerRegistry tracks known peers.
...
TransportClient is a thin wrapper around a backend.
...
MessageDispatcher routes decoded transport messages to sync actions.
...
For Ping, it returns a Pong.

For Ack, it calls:

SyncEngine::receive_ack(sync_id)

For SyncBatch, it decodes a sync operation and calls:
...
Submit a local operation through SyncEngine:
...
Get the next sync batch:

auto batch =
    sync_engine.next_batch();

Send the batch through transport:
...
Inbound messages are dispatched to MessageDispatcher.

Replies such as Pong and Ack are sent back automatically when possible.
...
It is used internally by TcpTransportBackend.
...
For now, the TCP backend is simple and blocking.

For production, the recommended direction is:
...
Linux sockets for the current TCP backend
Roadmap
...
softadastra/transport is the delivery layer for Softadastra sync.
